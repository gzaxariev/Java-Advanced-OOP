...
public class Main {
	public static void main(String[] args) throws IOException {
		...
		while (true){
			...
			getAllFieldsByModifier(modifier);
	...
	private static Field[] getAllFieldsByModifier(String modifier) {
		...
		switch (modifier){
			case "private":
				return Arrays.stream(declaredFields).filter(field ->Modifier.isPrivate(field.getModifiers()))
						.toArray(Field[]::new);
			case "protected":
				return Arrays.stream(declaredFields).filter(field -> Modifier.isProtected(field.getModifiers()))
						.toArray(Field[]::new);
			case "public":
				return Arrays.stream(declaredFields).filter(field -> Modifier.isPublic(field.getModifiers()))
						.toArray(Field[]::new);
			case "all":
